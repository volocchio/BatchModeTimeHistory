...
if summary_df is not None and len(summary_df) > 0:
    ...
    if "Payload-Range Curves" in plot_choices:
        ...
        with col1:
            ...
            if alt_options:
                ...
                sel_alt = None
        with col2:
            mach_options = sorted(filtered["mach"].unique().tolist(), reverse=True)
            if mach_options:
                sel_mach = st.selectbox("Mach", options=mach_options)
            else:
                sel_mach = None

        if sel_alt is not None and sel_mach is not None:
            df_pr = filtered[(filtered["cruise_alt"] == sel_alt) & (filtered["mach"] == sel_mach)]
            if len(df_pr) > 0:
                df_pr = df_pr.copy()
                ...
                fig.update_layout(
                    title=f"Payload-Range at FL{int(sel_alt/100):.0f}, M {sel_mach:.2f}",
        ...
        if alt_options:
            ...
            if len(df_mach) > 0:
                ...
                fig = px.line(
                    ...
                    line_dash="isa_label",
                    line_group="aircraft",
                    labels={"mach": "Mach", "total_dist_nm": "Max Range (NM)", "isa_label": "Temperature"},
                    title=f"Max Range vs Mach at FL{int(sel_alt/100):.0f}",
        ...
        if alt_options:
            ...
            if len(df_end) > 0:
                ...
                fig = px.line(
                    ...
                    line_dash="isa_label",
                    line_group="aircraft",
                    labels={"mach": "Mach", "total_time_min": "Max Endurance (min)", "isa_label": "Temperature"},
                    title=f"Max Endurance vs Mach at FL{int(sel_alt/100):.0f}",
                    color_discrete_map={"Flatwing": "red", "Tamarack": "green"},
                )
                fig.update_traces(mode="lines")
                st.plotly_chart(fig, use_container_width=True)
    ...
    if "Max Range vs Altitude" in plot_choices:
        ...
        mach_options = sorted(filtered["mach"].unique().tolist(), reverse=True)
        if mach_options:
            sel_mach = st.selectbox("Mach for Range vs Altitude", options=mach_options, key="rva_mach")
            df_alt = filtered[(filtered["payload"] == 0) & (filtered["mach"] == sel_mach)]
            if len(df_alt) > 0:
                # Create ISA deviation labels for better legend
                df_alt = df_alt.copy()
                ...
                fig = px.line(
                    ...
                    title=f"Max Range vs Altitude at M {sel_mach:.2f}",
                    color_discrete_map={"Flatwing": "red", "Tamarack": "green"},
                )
                fig.update_traces(mode="lines")
                st.plotly_chart(fig, use_container_width=True)
