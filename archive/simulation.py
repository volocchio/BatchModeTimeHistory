...
from utils import load_airports  # Add this line
...
def run_simulation(
    ...
    """
    Simulate a flight between two airports with the specified parameters, incorporating winds and temperatures aloft.
    ...
    Args:
        ...
        winds_temps_source: Source for winds and temps aloft ("Current Conditions", "Summer Average", "Winter Average").
        v1_cut_enabled: Boolean to enable V1 cut simulation (single engine operation).

    Returns:
        tuple: A tuple containing the flight simulation DataFrame, a dictionary of final results,
               and the latitude/longitude of departure and arrival airports.
    """
    ...
    while segment != 14:
        ...
        if alt - alt_to >= 35 and segment in (1, 2) and to_flag == 0:
            to_flag = 1
            dist_to_35ft = dist_ft - takeoff_roll_dist  # Distance from VR to 35 ft
            segment1_gradient = tan(gamma) * 100 if gamma != 0 else 0  # Gradient for segment 1
            # Add logging for debugging
            st.write(f"Takeoff - Ground Roll: {int(takeoff_roll_dist)} ft, Dist to 35 ft: {int(dist_to_35ft)} ft")
            if dist_to_35ft <= takeoff_roll_dist:
                st.warning("Adjusting dist_to_35ft: Distance to 35 ft should be greater than ground roll.")
                dist_to_35ft = takeoff_roll_dist * 1.5  # Arbitrary adjustme
        ...
        if alt - alt_to >= 400 and segment == 2:
            segment = 3
            dist_to_400ft = dist_ft - (takeoff_roll_dist + dist_to_35ft)  # Distance from 35 ft to 400 ft
            segment2_gradient = tan(gamma) * 100 if gamma != 0 else 0  # Gradient for segment 2
        if alt - alt_to >= 1500 and segment == 3 and v1_cut == 1:
            segment = 6
            dist_to_1500ft = dist_ft - (takeoff_roll_dist + dist_to_35ft + dist_to_400ft)  # Distance from 400 ft to 1500 ft
        ...
        if vkias <= 0 and segment == 14:
            ...
            segment = 14
            # Add logging for debugging
            dist_land_35 = dist_touchdown - dist_at_35  # Already calculated earlier
            st.write(f"Landing - Dist from 35 ft to Touchdown: {int(dist_land_35)} ft, Ground Roll: {int(dist_land)} ft")
            if dist_land_35 <= dist_land:
                st.warning("Adjusting dist_land_35: Distance from 35 ft to touchdown should be greater than ground roll.")
                adjusted_dist_at_35 = dist_touchdown - (dist_land * 1.5)  # Arbitrary adjustment for now
                dist_land_35 = dist_touchdown - adjusted_dist_at_35
                final_results["Dist from 35 ft (ft)"] = int(dist_land_35)  # Up
    ...
    if vkias <= 0 and segment == 14:
        dist_land_35 = dist_ft - dist_at_35
